...
for(i in 1:length(res.pen)){
    ...
    fdp.nls <- fdp.nls + sum(beta.nnls[-3] != 0) / sum(beta.true == 0)
    fnp.nls <- fnp.nls + sum(beta.nnls[3] == 0)
    fdp.sel <- fdp.sel + sum(beta.sel[-3] != 0) / sum(beta.true == 0)
    fnp.sel <- fnp.sel + sum(beta.sel[3] == 0)
    fdp.al <- fdp.al + sum(beta.al[-3] != 0) / sum(beta.true == 0)
    fnp.al <- fnp.al + sum(beta.al[3] == 0)
    fdp.lars <- fdp.sel + sum(beta.lars[-3] != 0) / sum(beta.true == 0)
    fnp.lars <- fnp.sel + sum(beta.lars[3] == 0)
}
