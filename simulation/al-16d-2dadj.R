...
filename <- paste("al-2dadj-sd02-", as.numeric(args[1]),".RData", sep = "")
load(nnlsname)
...
for(i in 1:length(dataRange)){
    ...
    dde.fit <- LS.sparse(DSIRfn.sparse, data = dsirData ,times.d, basisvals = basis.d, lambda = lambda, in.meth='nlminb', delay = delay, basisvals0 = basis0, coefs0 = coefs0, nbeta = 16, ndelay = 2, tau = list(seq(0,5, length.out = 16)), control.out = list(lambda.sparse = -3, pars.c = 50), nnls.res = nnls.res[[i]])
    al.res[[i]] <- dde.fit$select
    save(al.res, file = filename)
}
