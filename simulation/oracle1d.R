...
for(i in 1:100){
    ...
    initBeta <- 2 + initUnif[i]
    dde.fit <- Profile.LS.sparse(DSIRfn.sparse, dsirData, times.d, pars = initPars, beta = initBeta, coefs = coefs.d, basisvals = basis.d, lambda = 100, in.meth='nlminb', delay = delay, basisvals0 = basis0, coefs0 = coefs0, nbeta = length(initBeta), ndelay = 2, tau = list(c(1)), control.out = list(method = "nnls", maxIter = 10, lambda.sparse = 0))
    sim.res[[i]] <- dde.fit
    save(sim.res, DSIR.pars, file ="oracle1d.RData")
}
