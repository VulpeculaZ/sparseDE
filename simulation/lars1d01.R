...
for(i in 1:100){
    ...
    dde.fit <- LS.sparse(DSIRfn.sparse, data = sim.res[[i]]$data ,times.d, basisvals = basis.d, lambda = 100, in.meth='nlminb', delay = delay, basisvals0 = basis0, coefs0 = coefs0, nbeta = 16, ndelay = 2, tau = list(seq(0,5, length.out = 16)), control.out = list(method = "nnls", maxIter = 10, lambda.sparse = -1), nnls.res = sim.res[[i]]$res)
    sim.res[[i]] <- dde.fit$select
    save(sim.res, DSIR.pars, file ="pen1d01.RData")
}
