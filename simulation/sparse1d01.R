...
for(i in 1:length(nnls.res)){
    ...
    dde.fit <- LS.sparse(DSIRfn.sparse, data = nnls.res[[i]]$data ,times.d, basisvals = basis.d, lambda = 100, in.meth='nlminb', delay = delay, basisvals0 = basis0, coefs0 = coefs0, nbeta = 16, ndelay = 2, tau = list(seq(0,5, length.out = 16)), control.out = list(method = "nnls", maxIter = 10, lambda.sparse = -4), nnls.res = nnls.res[[i]]$res)
    sim.res[[i]] <- dde.fit$select
    save(sim.res, DSIR.pars, file ="lars2d01.RData")
}
